...
function resolve_resources(){
  ...
  for yaml in "$dir"/*.yaml; do
    ...
    sed -e "s+\(.* image: \)\(knative.dev\)\(.*/\)\(test/\)\(.*\)+\1\2 \3\4test-\5+g" \
        -e "s+ko://++" \
        -e "s+knative.dev/eventing/cmd/broker/ingress+${image_prefix}broker-ingress${image_tag}+" \
        -e "s+knative.dev/eventing/cmd/broker/filter+${image_prefix}broker-filter${image_tag}+" \
        ...
        -e "s+knative.dev/eventing/cmd/ping/jobrunner+${image_prefix}jobrunner${image_tag}+" \
