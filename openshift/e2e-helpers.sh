...
function tag_core_images(){
  ...
  oc policy add-role-to-group system:image-puller system:serviceaccounts:${EVENTING_NAMESPACE} --namespace=${OPENSHIFT_BUILD_NAMESPACE:-knative-eventing}
...
function tag_built_image() {
  ...
  local local_name=$2
  local build_tag=$3
#  oc tag --insecure=${INSECURE} -n ${EVENTING_NAMESPACE} ${OPENSHIFT_REGISTRY}/${NAMESPACE-ci-local}/stable:${remote_name} ${local_name}:${build_tag}
  oc tag --insecure=${INSECURE} -n ${EVENTING_NAMESPACE} "${IMAGE_FORMAT/\${component\}/${remote_name}}" "${local_name}:${build_tag}"
}
